...
// Returns the CVar with the given name, or nullptr if it does not exist.
std::shared_ptr<Ship::CVar> CVarGet(const char* name);

extern "C" {
#endif

// Typed getters: return the stored value, or defaultValue when the CVar is
...
// Typed setters: create the CVar if it does not exist.
...
// Registration: set the default only if the CVar is not already present.
